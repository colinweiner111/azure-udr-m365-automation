...
from function_app import _find_drifted_cidrs, _build_table_details
...
class TestRouteTableManager(unittest.TestCase):
    ...
    @patch('shared.route_manager.NetworkManagementClient')
    @patch('shared.route_manager.DefaultAzureCredential')
    def test_get_current_routes_uses_rg_table_key(self, mock_cred, mock_client):
        """Cross-RG entries should be keyed as 'resourcegroup/tablename'."""
        mock_route = Mock()
        mock_route.address_prefix = "203.0.113.0/24"
        mock_rt = Mock()
        mock_rt.routes = [mock_route]
        mock_client.return_value.route_tables.get.return_value = mock_rt
        ...
        self.assertIn("rg-spoke/rt-spoke", routes)
        self.assertEqual(routes["rg-spoke/rt-spoke"], ["203.0.113.0/24"])

    @patch('shared.route_manager.NetworkManagementClient')
    @patch('shared.route_manager.DefaultAzureCredential')
    def test_invalid_rg_table_entries_raise_value_error(self, mock_cred, mock_client):
        """Malformed rg/table entries should fail fast with clear validation."""
        for entry in ["/", "rg-only/", "/table-only"]:
...
class TestDriftDetection(unittest.TestCase):
    """Tests for per-route-table drift detection behavior."""

    def test_drifted_cidrs_detected_per_table(self):
        new_cidrs = ["1.1.1.0/24", "2.2.2.0/24"]
        to_remove = []
        current_routes_by_table = {
            "rg1/rt1": ["1.1.1.0/24", "2.2.2.0/24"],
            "rg2/rt2": ["1.1.1.0/24"],
            "rg3/rt3": [],
        }
        ...
        self.assertEqual(drifted, ["1.1.1.0/24", "2.2.2.0/24"])
        self.assertIn("rg2/rt2", missing_by_table)
        self.assertIn("rg3/rt3", missing_by_table)
        self.assertEqual(missing_by_table["rg2/rt2"], ["2.2.2.0/24"])
        self.assertEqual(missing_by_table["rg3/rt3"], ["1.1.1.0/24", "2.2.2.0/24"])


class TestRunLogFormatting(unittest.TestCase):
    """Tests for structured per-table run log formatting."""

    def test_build_table_details_merges_add_remove_and_missing(self):
        route_tables = [("rg1", "rt1"), ("rg2", "rt2")]
        add_summary = {
            "tables": {
                "rg1/rt1": {
                    "added": 2,
                    "failed": 1,
                    "errors": ["add-error"],
                    "added_routes": ["1.1.1.0/24", "1.1.2.0/24"],
                    "failed_routes": [{"cidr": "1.1.3.0/24", "error": "add-error"}],
                },
            }
        }
        remove_summary = {
            "tables": {
                ...
                "rg2/rt2": {
                    ...
                    "failed": 1,
                    "errors": ["remove-error"],
                    "removed_routes": ["10.0.1.0/24", "10.0.2.0/24", "10.0.3.0/24"],
                    "failed_routes": [{"cidr": "10.0.4.0/24", "error": "remove-error"}],
                },
            }
        }
        missing_by_table = {
            "rg1/rt1": ["1.1.1.0/24"],
            "rg2/rt2": ["2.2.2.0/24", "3.3.3.0/24"],
        }
        ...
        self.assertEqual(details["rg1/rt1"]["missing_before_run"], 1)
        self.assertEqual(details["rg1/rt1"]["added"], 2)
        self.assertEqual(details["rg1/rt1"]["add_failed"], 1)
        self.assertEqual(details["rg1/rt1"]["added_routes"], ["1.1.1.0/24", "1.1.2.0/24"])
        self.assertEqual(details["rg1/rt1"]["add_failed_routes"], [{"cidr": "1.1.3.0/24", "error": "add-error"}])
        self.assertEqual(details["rg1/rt1"]["removed"], 1)
        self.assertEqual(details["rg1/rt1"]["remove_failed"], 0)
        self.assertEqual(details["rg1/rt1"]["removed_routes"], ["10.0.0.0/24"])

        self.assertEqual(details["rg2/rt2"]["missing_before_run"], 2)
        self.assertEqual(details["rg2/rt2"]["added"], 0)
        self.assertEqual(details["rg2/rt2"]["add_failed"], 0)
        self.assertEqual(details["rg2/rt2"]["removed"], 3)
        self.assertEqual(details["rg2/rt2"]["remove_failed"], 1)
        self.assertEqual(details["rg2/rt2"]["removed_routes"], ["10.0.1.0/24", "10.0.2.0/24", "10.0.3.0/24"])
        self.assertEqual(details["rg2/rt2"]["remove_failed_routes"], [{"cidr": "10.0.4.0/24", "error": "remove-error"}])
        self.assertIn("remove-error", details["rg2/rt2"]["errors"])
...
class TestM365LiveIntegration(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.ROUTE_TABLE_KEY = f"{cls.RESOURCE_GROUP}/{cls.ROUTE_TABLE_NAME}"
    ...
    def test_add_real_m365_routes(self):
        ...
        current = set(routes_by_table[self.ROUTE_TABLE_KEY])
...
class TestRouteTableIntegration(unittest.TestCase):
    ...
    def test_add_route_creates_entry_in_azure(self):
        ...
        routes_by_table = self.manager.get_current_routes()
        cidrs = routes_by_table[self.ROUTE_TABLE_KEY]
        self.assertIn(
    ...
    def test_get_current_routes_returns_list(self):
        ...
        self.assertIn(self.ROUTE_TABLE_KEY, routes_by_table)
        self.assertIsInstance(routes_by_table[self.ROUTE_TABLE_KEY], list)
